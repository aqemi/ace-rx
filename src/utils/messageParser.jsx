...
export function parseReplies(text, onClick, onMouseEnter, onMouseLeave) {
  const onClickInternal = (event, match) => {
...
export function parseLinks(text) {
  return replacer(text, LINK_REGEXP, (match, i) => (
    <a href={match} target='_blank' rel='noopener noreferrer' key={`link${i}`}>
      {match}
    </a>
  ));
}
...
export function replaceYoutubeLink(text, title) {
  return replacer(text, YOUTUBE_REPLACE_REGEXP, (match, i) => (
    <a href={match} target='_blank' rel='noopener noreferrer' key={`youtube${i}`}>
      {title}
    </a>
  ));
}
