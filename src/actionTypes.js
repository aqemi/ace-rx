'use strict';

export const PLAYLIST_UPDATE = 'PLAYLIST_UPDATE';
export const PLAYLIST_SELECT = 'PLAYLIST_SELECT';
export const PLAYLIST_DESELECT = 'PLAYLIST_DESELECT';
export const PLAYLIST_PREVIOUS = 'PLAYLIST_PREVIOUS';
export const PLAYLIST_NEXT = 'PLAYLIST_NEXT';
export const PLAYLIST_UPLOAD_PROGRESS = 'PLAYLIST_UPLOAD_PROGRESS';
export const PLAYLIST_TOGGLE_SHUFFLE = 'PLAYLIST_TOGGLE_SHUFFLE';
export const PLAYLIST_CYCLE_REPEAT = 'PLAYLIST_CYCLE_REPEAT';
export const CHAT_UPDATE = 'CHAT_UPDATE';
export const CHAT_START = 'CHAT_START';
export const CHAT_STOP = 'CHAT_STOP';
export const CHAT_EMPTY = 'CHAT_EMPTY';
export const CHAT_LOG = 'CHAT_LOG';
export const POSTAREA_SET_UPLOADING = 'POSTAREA_SET_PROCESSING';
export const POSTAREA_SET_MESSAGE = 'POSTAREA_SET_MESSAGE';
export const POSTAREA_INSERT_REPLY = 'POSTAREA_INSERT_REPLY';
export const SNACKBAR_OPEN = 'SNACKBAR_OPEN';
export const SNACKBAR_CLOSE = 'SNACKBAR_CLOSE';
export const SHOW_PREVIEW = 'SHOW_PREVIEW';
export const HIDE_PREVIEW = 'HIDE_PREVIEW';
export const SET_ONLINE_COUNTER = 'SET_ONLINE_COUNTER';
export const IGNORE_ADD = 'IGNORE_ADD';
export const IGNORE_CLEAR = 'IGNORE_CLEAR';
export const IGNORE_LOAD = 'IGNORE_LOAD';
export const LOAD_TOPIC = 'LOAD_TOPIC';
export const AVATAR_LOAD = 'AVATAR_LOAD';
export const AVATAR_SET_UPLOADING = 'AVATAR_SET_UPLOADING';
export const LIGHTBOX_OPEN_IMAGE = 'LIGHTBOX_OPEN_IMAGE';
export const LIGHTBOX_OPEN_VIDEO = 'LIGHTBOX_OPEN_VIDEO';
export const LIGHTBOX_CLOSE = 'LIGHTBOX_CLOSE';
export const SETTINGS_OPEN = 'SETTINGS_OPEN';
export const SETTINGS_CLOSE = 'SETTINGS_CLOSE';
export const SETTINGS_SET = 'SETTINGS_SET';
