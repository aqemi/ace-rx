...
export const MARKUP_QUOTE_REGEXP = /(>(?:(?!\/\/|\n).)+|&gt;(?:(?!\/\/|\n).)+)/gm;
export const MARKUP_ADM_REGEXP = /(\$ADM\$)/g;
export const MARKUP_MOD_REGEXP = /(\$MOD\$)/g;
export const YOUTUBE_REGEXP = /(?:https?:\/\/)?(?:(?:www|music|m)\.)?(?:youtube\.com\/\S*(?:(?:\/e(?:mbed))?\/|watch\?(?:\S*?&?v\=))|youtu\.be\/)([a-zA-Z0-9_-]{6,11})(?:(?:.+?)t=(\d+))?/; //eslint-disable-line
export const YOUTUBE_REPLACE_REGEXP = /((?:https?:\/\/)?(?:(?:www|music|m)\.)?(?:youtube\.com\/\S*(?:(?:\/e(?:mbed))?\/|watch\?(?:\S*?&?v\=))|youtu\.be\/)(?:[a-zA-Z0-9_-]{6,11})(?:(?:.+?)t=(?:\d+))?)\S*/; //eslint-disable-line
